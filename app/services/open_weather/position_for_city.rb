class OpenWeather::PositionForCity < OpenWeather::BaseApiClient
  def call
    return fail!(error: "city, state and country are required") if city.blank? || city_state.blank? || country.blank?

    response = self.class.get("/geo/1.0/direct?#{query_params}")

    if !response.success? || response.parsed_response.empty?
      fail!(error: "please provide a valid city, state and country")
    end
  ...
  def query_params
    "q=#{city},#{city_state},#{country}&limit=1&appid=#{api_key}"
  end
end
